...
SimListData1 = list(
  ...
  splineMat = sp[1:length(unique(btdt$year)),1:ncol(sp)]
)
...
Inits1 = list(
  N = apply(btdt[,12:19], 1, max), 
  beta1 = rep(0.01,ncol(sp)), 
  beta0 = rep(0.01,length(unique(btdt$SITECODE))),
  delta0 = 0.01
...
Nestimates$siteind = rep(1:10,23)
Nestimates$yearind = rep(1:23,each=10)
...
Inits3 = list(
  N = apply(btdt[,12:19], 1, max), 
  beta1 = matrix(0.01,nrow = ncol(sp),ncol=length(unique(btdt$SITECODE))), # 6 knots so 7 splines
  beta0 = rep(0.01,length(unique(btdt$SITECODE))),
  delta0 = 0.01
)
...
# compare 
globalsamples$WAIC
shrinkgamsamples$WAIC # shrink mean wins but by less than with stan - however lower overall waic than the stan version
freegamsamples$WAIC

effectiveSize(shrinkgamsamples$samples) # lower effective sample size though - need to do more chains and more samples! 
...
SimListData1low = list(
  count = as.matrix(btdt[,12:19]),
  splineMat = sp6[1:length(unique(btdt$year)),1:ncol(sp6)]
)
# set up data 
SimCon1low = list(
  n = nrow(btdt),
...
SimListData1high = list(
  count = as.matrix(btdt[,12:19]),
  splineMat = sp8[1:length(unique(btdt$year)),1:ncol(sp8)]
)
# set up data 
SimCon1high = list(
  n = nrow(btdt),
