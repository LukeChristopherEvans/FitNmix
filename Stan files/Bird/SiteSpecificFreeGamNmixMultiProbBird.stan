...
data{
 ...
 int visitmatrix[n,maxvisits];
 int bandmatrix[n,maxvisits];
...
// re centre
 ...
 for (i in 1:n) {
  ...
  for(c in 1:visits[i]) {
     logitp[i,c] = delta0 + delta1 * visitmatrix[i,c]  +  delta2 * bandmatrix[i,c] ;
     }
 }
...
model {
 ...
 for (i in 1:n) {
   vector[k[i] - minpop[i] + 1] lp;
    ...
    for(j in minpop[i]:k[i]) { // evaluate by each visit
